...
Edited by Sinan Bayraktar, July 21
"""
...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    x  =  tf_util.conv2d(net, 64, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
                         scope='conv2', bn_decay=bn_decay)
    net = tf.concat(values=[x, net], axis=-1)
    net = tf_util.attention_block(net, is_training=is_training,
                         scope="SE2", r=16)
    x  =  tf_util.conv2d(net, 64, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
    ...
    net = tf.concat(values=[x, net], axis=-1)
    net = tf_util.attention_block(net, is_training, 
                         scope="SE3", r=16)
    x  =  tf_util.conv2d(net, 128, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
